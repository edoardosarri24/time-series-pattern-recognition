...
The main problem of the AoS in the GPU is given by the warp (i.e., a group of 32 threads) parallel execution: they execute the same instruction over different data. In particular, the first thread requires in order $t_0d_0,\cdots,t_0d_5$, the second requires $t_1d_0,\cdots,t_1d_5$, and so on. If all data with the same dimension are contiguous in memory, when the first thread loads its first dimension data in memory, it also loads the first dimension of the other threads; when these other threads search for their first dimension data in memory, the data will be already present.
