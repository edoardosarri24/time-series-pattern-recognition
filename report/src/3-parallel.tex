...
To ensure maintainability and separation of concerns, the parallel implementation is decoupled into a driver module and a specialized kernel module.

\subsection{Kernel Module Interface}
The kernel module encapsulates the GPU-specific logic, including the constant memory declaration and the kernel execution configuration. It exposes the following functional interface:

\begin{itemize}
    \item \texttt{pattern\_matching::update\_query\_constant(h\_query)}: A wrapper around \texttt{cudaMemcpyToSymbol} that abstracts the internal constant memory symbol, allowing the driver to update the query without direct access to device-specific variables.
    \item \texttt{pattern\_matching::launch\_kernel(input, output, N)}: Manages the kernel launch parameters (grid and block dimensions) and execution, ensuring that the \texttt{sad\_pattern\_matching} kernel is invoked correctly.
\end{itemize}

This decoupling ensures that \texttt{main.cpp} remains focused on high-level orchestration (data I/O, memory management, and result reduction) while the CUDA-specific implementation details are isolated in a \texttt{.cu} module.