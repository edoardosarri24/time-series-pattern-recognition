...
The main problem of the AoS in the GPU is given by the warp (i.e., a group of 32 threads) parallel execution: they execute the same instruction over different data. In particular, the first thread requires in order $t_0d_0,\cdots,t_0d_5$, the second requires data $t_1d_0,\cdots,t_1d_5$, and so on. If all data with the same dimension are contiguous in memory, when the first thread loads its first dimension data in memory, it also loads the first dimension of the other threads; when these other threads search for their first dimension data in memory, the data will be already present.
...
\begin{itemize}
    ...
    \item Constant Memory \\
        Stores the Query $Q$ in SoA layout. Since the constant memory is small (i.e., usually 64KB) the query cannot be too long: in our case, considering the default length of 64 described in Section~\ref{sec:dataset}, we have circa 1.5 KB, so it fits easily within the 64KB constant memory limit. This guarantees a huge performance benefit: we do not have to transport the query from the global memory to the local one for each thread; all threads in a warp use the same mamory area, so we can do a single memory transaction to have this data in the local memory.
    \item Shared Memory \\
