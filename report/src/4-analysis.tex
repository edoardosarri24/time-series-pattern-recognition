...
The motivation of this could be, in this case, the extra effort that the CPU must do to process the padding values, since they are the 25\% of the real data: if we work with a longer data length, probably the padding increases the speed of our code; in this case the cpu prefers to handle itself the load in memory of the unpadded data.
