...
To optimize the sequential execution, we implement an early abandoning approach: during the computation of $D(i)$, we maintain a the current distance; if the partial local sum exceeds the current global minimum distance found ($min\_dist$), the computation for the current window $i$ is immediately halted.