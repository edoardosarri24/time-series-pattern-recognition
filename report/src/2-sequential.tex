...
\begin{enumerate}
    \item Data Loading \\
        We allocate memory and load in it the input file. We reorganized the data into a Array of Structures (AoS) memory pattern.
    \item Query Generation \\
        We extract a subsequence of lenght $M$ as the query. As described in Section~\ref{sec:input}, we add to this subsequence a Gaussin noice to avoid that the SAD distance is $SAD=0$, i.e. that the query is not exactly the same of our input subsequence. In the code there is a seed to allow the reproducibility of the code; we use the seed 78 for our experiments.
    \item Pattern Matching \\
        We use a sliding window to scan the input, calculating a SAD of lenght $M$ for each istant time.
    \item Reporting \\
        We return the index of the best match and the relative SAD value.
\end{enumerate}
...
The SAD metric calculation is performed using a Brute Force Sliding Window approach. If $Q$ be the query of length $M$, we search for the starting index $i^*$ that minimizes the distance: $\displaystyle i^*=\arg\max_{i\in[1,N-M]}D(i)$, where = $\displaystyle D(i)=\sum_{m=1}^{M} \sum_{d=1}^{D} |Q[m,d] - T[i+m,d]|$.

\myskip

To optimize the sequential execution, we implement an early abandoning approach: we mantain the during the computation of $D(i)$, we maintain a running sum of the distance. If at any point the partial sum exceeds the current global minimum distance found so far ($min\_dist$), the computation for the current window $i$ is immediately halted.