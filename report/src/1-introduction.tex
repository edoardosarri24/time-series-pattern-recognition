...
\label{sec:input}
To define the input and the query we have to address two thinks:
\begin{itemize}
    \item Input \\
    ...
    \item Query \\
        We sample a casual index of the input and take a query of Lenght $M$. The choice for the lenght of the action is $M=64$: we are evaluating 1,28 second of data and search the closest part of action in the dataset. We add to this query a Gaussian noise to ensure that $SAD > 0$. This perturbation is obtain from a Normal Distribution with $\mu=0$ and $\sigma=0,1$; this garantee the robustness of our approach.
\end{itemize}
